...
function CreateSiloModalForm(props: CreateSiloModalFormProps) {
  ...
  const { register, handleSubmit, control, setValue, formState: { errors } } = useForm<SiloFormInputs>({
    defaultValues: {
      members: [],
      state: false
    }
  ...
  useEffect(() => {
    fetch('http://localhost:8123/ws/feeds', {
      method: 'GET',
    })
  }, [])
  ...
  const onSubmit: SubmitHandler<SiloFormInputs> = data => {
    fetch(`http://localhost:8123/api/createSilo`, {
      method: "POST",
